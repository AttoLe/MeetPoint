...
export class SessionMapSectionComponent {
  ...
  /*private advancedMarkers = computed<
  ...
  >(() => {
    ...
    return this.participants().map((user) => {
      ...
      if (user.avatar) {
        ...
        markerDiv.textContent = user.username;
      }
  ...
  /*effect(() => {
      var user = this._userService.user();
      if (user) {
        const mapped = this.users.map((u) => ({
          id: u.id.toString(),
          position: u.position,
          username: u.name,
        }));
        this.participants.update((v) => [...v, ...mapped]);
      }
    });*/
  /*clearMarkers() {
  ...
  private async fetchNearbyPlaces(
    ...
    // Clear all non-user markers before fetching new places
    //this.markers.sete = [{ position: location, title: 'You are here' }];
